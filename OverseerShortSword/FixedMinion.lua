...
function FindTarget()
	...
	for i, v in pairs(Players:GetChildren()) do
		if v:IsA("Player") and (not Creator or (Creator and v ~= Creator.Value)) and v.Character then
			...
			local ttorso = character:FindFirstChild("Torso")
			local HRP = humanoid.RootPart
			local TorsoDistance
			if ttorso then
			    TorsoDistance = (ttorso.Position - Torso.Position).magnitude
			else
			   TorsoDistance =  (HRP.Position - Torso.Position).magnitude
			end
			if humanoid and humanoid.Health > 0 and ttorso and TorsoDistance <= ClosestTorsoDistance then
